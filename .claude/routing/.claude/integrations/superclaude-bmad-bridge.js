...
exports.SuperClaudeBMADBridge = void 0;
exports.getSuperClaudeBMADBridge = getSuperClaudeBMADBridge;
exports.resetBridge = resetBridge;
const bmad_agent_system_1 = require("../../lib/agents/bmad-agent-system");
const delegation_coordinator_1 = require("../../lib/agents/delegation-coordinator");
const session_manager_1 = require("../../lib/agents/session-manager");
class SuperClaudeBMADBridge {
    constructor(config = {}) {
        this.config = {
            autoActivation: true,
    ...
    async initializeBMADSystem() {
        try {
            ...
            this.bmadSystem = new bmad_agent_system_1.BMADSystem();
            this.sessionManager = new session_manager_1.AdvancedSessionManager();
            this.delegationCoordinator = new delegation_coordinator_1.DelegationCoordinator(this.bmadSystem, this.sessionManager);
    ...
    async analyzeRequestForDelegation(request) {
        const analysis = {
    ...
    calculateRequestComplexity(request) {
        ...
        if (context.flags) {
            if (context.flags.includes('--think'))
                complexity += 0.2;
            if (context.flags.includes('--think-hard'))
                complexity += 0.3;
            if (context.flags.includes('--ultrathink'))
                complexity += 0.4;
            if (context.flags.includes('--delegate'))
                complexity += 0.2;
            if (context.flags.includes('--wave-mode'))
                complexity += 0.3;
        }
    ...
    mapPersonaToBMADAgents(persona) {
        if (!persona)
            return [];
    ...
    async delegateToAgents(request, analysis) {
        ...
        try {
            ...
            const result = await this.delegationCoordinator.delegateTask(sessionId, delegationRequest, options);
            return result;
        }
        catch (error) {
    ...
    mapToBMADRequestType(request, analysis) {
    ...
    selectAggregationMethod(delegationType) {
    ...
    async refreshConfiguration(newConfig) {
...
exports.SuperClaudeBMADBridge = SuperClaudeBMADBridge;
// Singleton instance for global use
let bridgeInstance = null;
function getSuperClaudeBMADBridge(config) {
    if (!bridgeInstance) {
