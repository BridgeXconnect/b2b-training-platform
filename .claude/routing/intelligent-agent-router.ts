...
import { SuperClaudeBMADBridge, SuperClaudeRequest, SuperClaudeResponse } from '../integrations/superclaude-bmad-bridge';
import { AgentIntegrationHook } from '../hooks/agent-integration-hook';

export interface RouteTarget {
  id: string;
  type: 'bmad' | 'mcp' | 'archon' | 'tmux' | 'claude-code';
  endpoint: string;
  capabilities: string[];
  priority: number;
  healthScore: number;
...
export interface RoutingCondition {
  type: 'complexity' | 'persona' | 'flags' | 'content' | 'performance' | 'availability';
...
export interface RoutingMetrics {
  ...
  routesByTarget: Map<string, number>;
  performanceByTarget: Map<string, { avgTime: number; successRate: number }>;
}

export class IntelligentAgentRouter {
  private targets: Map<string, RouteTarget>;
  private rules: Map<string, RoutingRule>;
  private metrics: RoutingMetrics;
  private bmadBridge: SuperClaudeBMADBridge;
  private agentHook: AgentIntegrationHook;
  ...
  private async initialize() {
    try {
      ...
      this.bmadBridge = new SuperClaudeBMADBridge();
      this.agentHook = new AgentIntegrationHook();
  ...
  private async loadRoutingTargets() {
    // BMAD System targets
    this.addTarget({
      id: 'bmad_system',
      type: 'bmad',
   ...
   */
  async route(request: SuperClaudeRequest): Promise<RoutingDecision> {
  ...
  private evaluateRules(request: SuperClaudeRequest): Map<string, number> {
  ...
  private evaluateCondition(condition: RoutingCondition, request: SuperClaudeRequest): number {
  ...
  private calculateComplexity(request: SuperClaudeRequest): number {
    // Use similar logic to SuperClaude bridge
    let complexity = 0.1;
    
    const content = request.content.toLowerCase();
    if (content.length > 500) complexity += 0.2;
    if (content.includes('create') && content.includes('analyze')) complexity += 0.3;
    if (request.context.flags?.includes('--think')) complexity += 0.2;
    if (request.context.flags?.includes('--think-hard')) complexity += 0.3;
    
    return Math.min(complexity, 1.0);
  }

  private scoreTargets(request: SuperClaudeRequest, ruleMatches: Map<string, number>): Map<string, number> {
  ...
  private calculateCapabilityMatch(request: SuperClaudeRequest, capabilities: string[]): number {
    const content = request.content.toLowerCase();
    let matches = 0;
  ...
  private selectOptimalTarget(scores: Map<string, number>, request: SuperClaudeRequest): RoutingDecision {
  ...
  private calculateRiskScore(target: RouteTarget, confidence: number): number {
    let risk = 0;
   ...
   */
  async executeRoute(decision: RoutingDecision, request: SuperClaudeRequest): Promise<SuperClaudeResponse> {
  ...
  private async executeOnTarget(targetId: string, request: SuperClaudeRequest): Promise<SuperClaudeResponse> {
    ...
    try {
      let result: SuperClaudeResponse;
  ...
  private async executeBMADRequest(request: SuperClaudeRequest): Promise<SuperClaudeResponse> {
    return await this.bmadBridge.processRequest(request);
  }

  private async executeMCPRequest(targetId: string, request: SuperClaudeRequest): Promise<SuperClaudeResponse> {
  ...
  private async executeArchonRequest(request: SuperClaudeRequest): Promise<SuperClaudeResponse> {
  ...
  private async executeTmuxRequest(targetId: string, request: SuperClaudeRequest): Promise<SuperClaudeResponse> {
  ...
  private async executeClaudeCodeFallback(request: SuperClaudeRequest): Promise<SuperClaudeResponse> {
  ...
  // Helper methods
  private addTarget(target: RouteTarget) {
    this.targets.set(target.id, target);
    this.circuitBreakers.set(target.id, new CircuitBreaker(target.id));
  }

  private addRule(rule: RoutingRule) {
    this.rules.set(rule.id, rule);
  }

  private checkCache(request: SuperClaudeRequest): CachedRoute | null {
    const cacheKey = this.generateCacheKey(request);
    return this.cache.get(cacheKey) || null;
  }

  private cacheRoute(request: SuperClaudeRequest, decision: RoutingDecision) {
    const cacheKey = this.generateCacheKey(request);
  ...
  private generateCacheKey(request: SuperClaudeRequest): string {
  ...
  private isCacheExpired(cached: CachedRoute): boolean {
    const maxAge = 5 * 60 * 1000; // 5 minutes
    return (Date.now() - cached.timestamp) > maxAge;
  }

  private updateTargetMetrics(targetId: string, success: boolean, responseTime: number) {
    const target = this.targets.get(targetId);
    if (!target) return;
  ...
  private updateCircuitBreaker(targetId: string, success: boolean) {
    const circuitBreaker = this.circuitBreakers.get(targetId);
    if (circuitBreaker) {
      if (success) {
  ...
  private updateTargetHealth(targetId: string, health: number) {
    const target = this.targets.get(targetId);
    if (target) {
  ...
  async healthCheck() {
    const targetHealth = new Map<string, boolean>();
...
class CircuitBreaker {
  ...
  constructor(private targetId: string, private threshold = 5, private timeout = 60000) {}
...
class HealthMonitor {
  ...
  startMonitoring(targets: Map<string, RouteTarget>, updateCallback: (id: string, health: number) => void) {
  ...
  private async checkTargetHealth(target: RouteTarget): Promise<number> {
...
export function getIntelligentAgentRouter(): IntelligentAgentRouter {
  if (!routerInstance) {
