...
import { IntelligentAgentRouter, RoutingDecision } from '../routing/intelligent-agent-router';
...
export class AgentOrchestrator {
  ...
  async orchestrateRequest(
    ...
    try {
      ...
      if (this.config.fallbackToClaudeCode) {
        const fallbackResult = await this.executeClaudeCodeFallback(
          this.normalizeRequest(request, context)
        );
        return this.createOrchestrationResult(
          fallbackResult,
          false,
  ...
  private normalizeRequest(request: string | object, context: any): SuperClaudeRequest {
    ...
    if (typeof request === 'string') {
      ...
      content = (request as any).content || (request as any).message || JSON.stringify(request);
      type = (request as any).type || this.inferRequestType(content);
    }
  ...
  private inferRequestType(content: string): string {
  ...
  private inferPersona(content: string): string | undefined {
    const lowerContent = content.toLowerCase();
  ...
  private inferFlags(content: string): string[] {
    const flags: string[] = [];
    const lowerContent = content.toLowerCase();
  ...
  private calculateComplexity(content: string): number {
    ...
    const multiStepCount = multiStepWords.filter(word => 
  ...
  private inferPriority(content: string): 'low' | 'medium' | 'high' | 'urgent' {
    const lowerContent = content.toLowerCase();
  ...
  private async routeToAgentEcosystem(request: SuperClaudeRequest): Promise<SuperClaudeResponse> {
    try {
  ...
  private checkCache(request: SuperClaudeRequest): CachedResult | null {
  ...
  private cacheResult(request: SuperClaudeRequest, result: SuperClaudeResponse) {
    const cacheKey = this.generateCacheKey(request);
  ...
  private updateAgentUtilization(agentsUsed: string[]) {
  ...
  private setupGlobalInterception() {
    ...
    if (typeof global !== 'undefined') {
      ...
      (global as any).__claudeCodeOrchestrator = this;
  ...
  async updateConfiguration(newConfig: Partial<OrchestratorConfig>) {
...
export function processClaudeCodeRequest(request: string | object, context: any = {}) {
...
if (typeof global !== 'undefined') {
  (global as any).processClaudeCodeRequest = processClaudeCodeRequest;
}